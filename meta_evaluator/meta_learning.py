"""Meta-learning loop for MetaEvaluator."""
...
class MetaEvaluatorLearner:
    ...
    def _prepare_params(self, *, detach: bool = False) -> List[torch.Tensor]:
        ...
        if detach:
            return MetaEvaluator.clone_parameters(params)
        return list(params)
    ...
    def _evaluate_task(
        ...
        *,
        ...
        """Evaluate a single task with optional context initialization and weight adaptation.
    ...
    def build_context_bank(
        ...
        *,
        ...
        """Adapt contexts for each training task to seed later reuse.
    ...
    def evaluate(
        ...
        *,
        ...
        Args:
            ...
            context_bank: Optional bank for per-model context initialization.
        ...
        context_by_model = {entry.model_name: entry.context for entry in context_bank} if context_bank else {}
        for task in tqdm(tasks_list, desc="Meta-eval", leave=False):
            init_context = context_by_model.get(task.model_name)
            preds = self._evaluate_task(
                ...
                target_descriptor=task.query_descriptor,
                target_label=task.query_label,
                adapt_descriptor=task.support_descriptor,
                adapt_label=task.support_label,
            )
    ...
    def evaluate_transfer(
        ...
        *,
        ...
        context_by_model = {entry.model_name: entry.context for entry in context_bank} if context_bank else {}
        for task in tqdm(tasks_list, desc="Transfer-eval", leave=False):
            ...
            preds = self._evaluate_task(
                ...
                adapt_context_steps=adapt_context_steps,
                adapt_weight_steps=adapt_weight_steps,
                target_descriptor=task.transfer_descriptor,
