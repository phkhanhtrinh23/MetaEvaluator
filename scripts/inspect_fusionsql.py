"""Inspect a saved FusionSQL meta-learner checkpoint."""
...
from fusion_sql.model import FusionSQL  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--checkpoint", default="outputs/fusionsql/fusionsql_model.pt", help="Path to FusionSQL checkpoint (.pt).")
...
def main() -> None:
    ...
    model = FusionSQL(input_dim=args.input_dim, hidden_dims=(args.hidden1, args.hidden2))
    model.load_state_dict(state)

    print("FusionSQL architecture:")
    print(model)
