"""End-to-end pipeline to train and evaluate MetaEvaluator."""
...
def parse_args() -> argparse.Namespace:
    """Build CLI for the MetaEvaluator pipeline and return parsed args.
    ...
    parser.add_argument("--output-dir", default="outputs/meta_evaluator", help="Directory for MetaEvaluator artifacts.")
    parser.add_argument("--embedding-dir", default="outputs/meta_evaluator/embeddings", help="Embedding cache directory.")
...
def run_inference_for_models(
    ...
    *,
    ...
    for model in models:
        model_name = model.alias or model.model_id
        print(f"[MetaEvaluator] Generating SQL for {model_name}...")
...
def main() -> None:
    ...
    model = MetaEvaluator(input_dim=len(DEFAULT_FEATURE_ORDER) + args.context_dim)
    ...
    meta_learner = MetaEvaluatorLearner(model, meta_cfg)

    print(f"[MetaEvaluator] Training on {len(train_tasks)} tasks for {args.epochs} epochs.")
    checkpoint_path = output_dir / "meta_evaluator_model.pt"
    ...
    print(f"[MetaEvaluator] Meta-test MAE: {meta_mae:.4f}")
    ...
    if transfer_mae is not None:
        print(f"[MetaEvaluator] Real-test MAE: {transfer_mae:.4f}")
    ...
    save_json(output_dir / "meta_evaluator_metrics.json", metrics)
    if args.save_meta_preds:
        save_json(output_dir / "meta_evaluator_meta_predictions.json", {"results": meta_results})
    save_json(output_dir / "meta_evaluator_dev_predictions.json", {"results": transfer_results})
    if test_models:
        print(f"[MetaEvaluator] Adapting to {len(test_models)} held-out model(s).")
        ...
        if test_meta_mae is not None:
            print(f"[MetaEvaluator] Held-out Meta-test MAE: {test_meta_mae:.4f}")
        if test_dev_mae is not None:
            print(f"[MetaEvaluator] Held-out Real-test MAE: {test_dev_mae:.4f}")
        save_json(output_dir / "meta_evaluator_test_meta_predictions.json", {"results": test_meta, "mae": test_meta_mae})
        save_json(output_dir / "meta_evaluator_test_dev_predictions.json", {"results": test_dev, "mae": test_dev_mae})
