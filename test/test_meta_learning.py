...
from meta_evaluator.descriptors import DEFAULT_FEATURE_ORDER, ShiftDescriptor
from meta_evaluator.evaluation import build_prediction_records
from meta_evaluator.meta_learning import MetaEvaluatorLearner, MetaLearningConfig, ShiftDescriptorTask
from meta_evaluator.model import MetaEvaluator
...
def test_meta_evaluator_forward_matches_functional():
    model = MetaEvaluator(input_dim=len(FEATURES), dropout=0.0)
    model.eval()
    torch.manual_seed(0)
...
def test_meta_learner_recovers_synthetic_accuracy():
    ...
    context_dim = 4
    model = MetaEvaluator(input_dim=len(FEATURES) + context_dim, dropout=0.0)
    ...
    learner = MetaEvaluatorLearner(model, cfg)
